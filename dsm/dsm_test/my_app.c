...
int sum = 0;
int cur_index = 0;
int operation_mode = 0; // 0 for read, 1 for write
...
void *thread_func(void *argp) {
    ...
    while (1) {
    	...
    	printf("Enter operation mode (0 for READ, 1 for WRITE): ");
    	scanf("%d", &operation_mode);

    	while (operation_mode != 0 && operation_mode != 1) {
    		printf("Invalid mode. Please enter 0 for READ or 1 for WRITE: ");
    		scanf("%d", &operation_mode);
    	}
